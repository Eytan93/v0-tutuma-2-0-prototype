      ...
      <div className="max-w-7xl mx-auto">
          ...
          {cart.length > 0 && (
            <div className="flex items-center gap-3 bg-[#1A1A1A] text-[#F3F0DF] px-6 py-3">
              ...
              <span className="text-xs tracking-[0.2em] uppercase font-sans">
                {cart.length} ítem{cart.length > 1 ? 's' : ''} · ${total.toLocaleString()} MXN
              </span>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-px bg-[#C8C4B0]">
          {filtered.map((product) => {
            ...
            const brandColor = brands.find((b) => b.id === product.brandId)?.color ?? '#6B7F5E'
            return (
              <div key={product.id} className="bg-[#F3F0DF] p-6 flex flex-col gap-4">
                ...
                <div>
                  ...
                  <h3 className="font-serif text-xl text-[#1A1A1A] leading-tight">
                    {product.name}
                    {product.year && (
                      <span className="text-[#1A1A1A]/30 font-normal"> {product.year}</span>
                    )}
                  </h3>
                </div>
                <div className="mt-auto pt-4 flex items-center justify-between border-t border-[#C8C4B0]">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 text-xs tracking-[0.15em] uppercase transition-all duration-300 font-sans ${
                      isAdded
